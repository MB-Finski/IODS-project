...
exposure is equal to the OR of an exposure given an outcome). This means that the above strategy is valid for scoping out potential regressors. 

As a side note, beyond the scope of this chapte, we may be missing out on potential interactions in the data
...
my_huge_reg_model <- glm(high_use ~ failures + absences + sex, data = bare_dataset, family = "binomial")

cv.glm(data = bare_dataset,
                          cost = cost_function,
                          glmfit = my_huge_reg_model,
                          K = 10)$delta[1]
...
As is evident from the comparison graph, the increasing number of regressors leads to overfitting of the data. I.e. 
the models become too complex and begin to describe spurious rather than true associations in the data. The "sweet spot"
for the number of regressors in our data seems to be around 5 or 6 predictors/regressors.